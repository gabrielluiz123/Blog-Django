...
class PostBusca(PostIndex):
    ...
    def get_queryset(self):
        ...
        if not termo:
            return qs
        qs = qs.filter(
            Q(titulo_post__icontains=termo) | Q(autor_post__username__iexact=termo)|Q(conteudo_post__icontains=termo)|
            Q(excerto_post__icontains=termo) | Q(categoria_post__nome_cat__iexact=termo)
        )
...
class PostDetalhes(UpdateView):
    template_name = 'posts/detalhes.html'
    model = Post
